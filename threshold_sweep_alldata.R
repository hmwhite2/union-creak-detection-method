...
sweep$thres <- gsub("sample\\d+_passage_", "", sweep$thres) # adjust regex to fit your file names
...
for(i in 1:length(files)){
  temp_data <- tg.read(files[i]) %>%
    tg.removeTier("ORT-MAU") %>% tg.removeTier("KAN-MAU") %>% tg.removeTier("KAS-MAU") %>% tg.removeTier("MAU") %>% tg.removeTier("SON") %>% tg.removeTier("MAS") %>% tg.removeTier("TRN") # remove additional tiers in your textgrid
