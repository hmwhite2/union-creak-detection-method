...
### file names. These are noted in the script on lines 118.

### To run this script you need to have run create_pm_files on your data and you will need 
### a durations.txt file which can be made with the get_durations.praat script.
...
below.am %>% 
  filter(localf0 > 0) -> below.am
below.am$time <- round(below.am$time, digits = 2)
below.am %>% 
  group_by(speaker, time) %>% 
  mutate(f0_new = mean(localf0)) %>% 
  select(speaker, time, f0_new, antimode) %>% 
  rename(localf0 = f0_new) %>% 
  ungroup() %>% 
  group_by(speaker) %>% 
  arrange(speaker, time) %>% 
  unique(.) -> below.am
...
pm_files %>% 
  ...
  unique(.) -> pm_files
below.am <- merge(below.am, pm_files, by = c("speaker", "time"), all.y = T)
...
below.am$below_am[is.na(below.am$below_am)] <- 0
below.am$antimode <- NULL
