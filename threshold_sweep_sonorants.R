...
for(i in 1:length(files)){
  temp_data <- tg.read(files[i]) %>%
    tg.removeTier("ORT-MAU") %>% tg.removeTier("KAN-MAU") %>% tg.removeTier("KAS-MAU") %>% tg.removeTier("MAS") %>% tg.removeTier("TRN") %>% tg.removeTier("creak")
...
phonemes_df$speaker <- gsub("_\\w-hc", "", phonemes_df$speaker) # adjust regex to fit your file names
...
for(i in 1:length(files)){
  temp_data <- tg.read(files[i]) %>%
    tg.removeTier("ORT-MAU") %>% tg.removeTier("KAN-MAU") %>% tg.removeTier("KAS-MAU") %>% tg.removeTier("MAS") %>% tg.removeTier("TRN") %>% tg.removeTier("MAU")
